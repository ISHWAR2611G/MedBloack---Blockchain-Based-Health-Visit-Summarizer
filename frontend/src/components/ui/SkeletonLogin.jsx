/**
 * MedBlock — SkeletonLogin
 * Loading skeleton shown while the Login page initializes.
 * Ported from Login Project's SkeletonLogin.tsx to plain JSX.
 *
 * Features:
 *  ✅ Keyframes injected once — no external CSS needed
 *  ✅ Matches real Login layout (logo, role pills, inputs, button, trust badges)
 *  ✅ Staggered shimmer — each block animates slightly offset
 *  ✅ Pulse glow on card border
...
function shimmerStyle(width, height, delay = 0, borderRadius = 8) {
  return {
    width,
    height,
    borderRadius,
...
function Row({ children, gap = 8, mb = 0, style }) {
  return (
...
function Block({ delay = 0, width = '100%', height = 36, borderRadius = 8 }) {
  return <div style={shimmerStyle(width, height, delay, borderRadius)} />;
}
