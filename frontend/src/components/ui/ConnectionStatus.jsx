/**
 * MedBlock — ConnectionStatus
 * Live blockchain/MetaMask status bar shown at bottom-left of login page.
 * Ported from Login Project's ConnectionStatus.tsx to plain JSX.
 *
 * Features:
 *  ✅ Real-time blockchain online/offline detection
 *  ✅ MetaMask wallet connection status
 *  ✅ Network name (Ethereum, Polygon, Hardhat, etc.)
 *  ✅ Current block number
 *  ✅ Click account address to copy
 *  ✅ Cleanup on unmount
 */
...
if (typeof document !== 'undefined' && !document.getElementById(SPIN_STYLE_ID)) {
  ...
  s.textContent = `
    ...
    @keyframes mb-pulse-dot { 0%,100% { opacity: 1; } 50% { opacity: 0.3; } }
    .mb-spin  { animation: mb-spin  1s linear infinite; }
    .mb-pulse-dot { animation: mb-pulse-dot 1.8s ease-in-out infinite; }
  `;
...
function StatusPill({ children, dotColor, pulse, onClick, title }) {
  return (
    <div
      title={title}
      onClick={onClick}
      ...
      onMouseEnter={(e) => {
        if (onClick) e.currentTarget.style.background = 'rgba(30,41,59,0.95)';
      }}
      onMouseLeave={(e) => {
        if (onClick) e.currentTarget.style.background = 'rgba(15,23,42,0.88)';
      }}
    >
      {dotColor && (
        <span
          className={pulse ? 'mb-pulse-dot' : undefined}
          style={{
            width:        7,
            height:       7,
            borderRadius: '50%',
